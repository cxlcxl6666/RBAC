			  ...
			  <div class="panel-body">
				<form id="roleForm" role="form" class="form-inline">
				  ...
				  <div class="form-group">
					...
					<select name="assginRoleids" id="leftList" class="form-control" multiple size="10" style="width:150px;overflow-y:auto;">
				  ...
				  <div class="form-group" style="margin-left:40px;">
					...
					<select name="unassignRoleids" id="rightList" class="form-control" multiple size="10" style="width:150px;overflow-y:auto;">
            ...
            $(function () {
							...
							$.ajax({
								...
								success:function(result){
									if(result.success){
										...
										layer.msg("分配角色数据成功", {time : 1000,icon : 6,shift : 6}, function() {});
									}else{
			    ...
			    $("#right2LeftBtn").click(function(){
			    	var opts = $("#rightList :selected");
						if(opts.length == 0){
							layer.msg("请选择需要移除的角色数据", {time : 2000,icon : 5,shift : 6}, function() {});
						}else{
						/* 	alert($("#roleForm").serialize())
							return; */
							//发送ajax请求
							$.ajax({
								type:"POST",
								url:"${APP_PATH}/user/deleteAssignRole",
								data:$("#roleForm").serialize(),
								success:function(result){
									if(result.success){
										$("#leftList").append(opts);
										layer.msg("移除分配角色数据成功", {time : 1000,icon : 6,shift : 6}, function() {});
									}else{
										layer.msg("移除分配角色数据失败", {time : 2000,icon : 5,shift : 6}, function() {});
									}
								}
							});
						}
