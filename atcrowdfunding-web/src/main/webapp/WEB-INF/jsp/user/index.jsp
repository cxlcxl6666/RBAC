...
<body>
	...
	<script type="text/javascript">
				...
				success : function(result) {
					...
					if (result.success) {
						...
						$.each(users,function(i, user) {
							...
							tableContent += '	<td> ';
							tableContent += '		<button type="button" onclick="goAssidnPage('+ user.id+ ')" class="btn btn-success btn-xs">';
							tableContent += '			<i class=" glyphicon glyphicon-check"></i> ';
							tableContent += '		</button> ';
							tableContent += '		<button type="button" onclick="goUpdatePage('+ user.id+ ')" class="btn btn-primary btn-xs">';
						...
						if (pageno > 1) {
							pageContent += '<li ><a href="#" onclick="pageno('
									+ (pageno - 1) + ')">上一页</a></li>';
						}
						...
						for (var i = 1; i <= userPage.totalno; i++) {
							if (i == pageno) {
								...
								pageContent += '<li><a href="#" onclick="pageno('+ i + ')">' + i + '</a></li>';
							}
						}
	
						if (pageno < userPage.totalno) {
							pageContent += '<li><a href="#" onclick="pageno('+ (pageno + 1) + ')">下一页</a></li>';
						}
		...
		function deleteUser(id, loginacct) {
			layer.confirm("确定删除" + loginacct + "的信息吗？", {icon : 3,title : '提示'}, function(cindex) {
		...
		function deleteUser() {
			...
			if (boxs.lenth == 0) {
				...
				layer.confirm("确定删除选择的用户信息吗？", {icon : 3,title : '提示'}, function(cindex) {
					...
					$.ajax({
						...
						data : $("#userData :checked").serialize(),
						befoerSend : function() {
							loadingIndex = layer.msg('处理中', {icon : 16});
						},
						success : function(result) {
							layer.close(loadingIndex);
							if (result.success) {
								layer.msg("删除的用户信息成功", {time : 1000,icon : 6,shift : 6}, function() {});
								window.location.href="${APP_PATH}/user/index";
							} else {
								layer.msg("删除的用户信息失败", {time : 2000,icon : 5,shift : 6}, function() {});
								return;
							}
						}
					});
